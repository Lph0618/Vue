          ...
          <div id="p1" class="col-md-7">
            ...
            <div class="card border-0 flex-md-row box-shadow h-md-250">
              <div class="card-body d-flex flex-column align-items-start">
                ...
                <!-- 地址这里有问题: 在Vue中跳转是用router-link标签和to 且传参的方式是 /页面名/参数 -->
                <!-- 所以:传参那个是需要拼接的,截取出原数据中需要的部分 -->
              ...
              <div class="col-md-4 p-0 pl-2" v-for="(p,i) of others" :key="i">
                <div class="card border-0 text-center">
                  ...
                  <div class="card-body p-0 pr-1 pl-1">
                    <span class="d-inline-block" v-text="p.title">华硕RX310 金属超极本</span>
...
export default {
  data(){
    return {
      //为什么要给值:因为axios是异步请求,是有延迟的,暂时没有数据,而页面上用该类型的API就会报错
      p1:{ price:0, href:"" }, // 保存第一个商品的详情信息
      p2:{ price:0, href:"" }, // 保存第二个商品的详情信息
      p3:{ price:0, href:"" }, // 保存第三个商品的详情信息
      //为什么数组不需要默认值: 因为v-for循环遍历,而如果数组的length为0,则不会去遍历
      others:[], // 保存后3个商品的详情信息
    }
  },
  ...
  mounted(){
    ...
    getIndex().then(result=>{
      console.log(result);
      // 将获取到的前3个商品,用解构分别保存到对象中
      [this.p1,this.p2,this.p3] = result;
      // 将获取到的后3个商品,用解构+截取保存进数组中
